...
class MonopedState(object):
    ...
    def __init__(self, max_height,interval, min_height, abs_max_roll, abs_max_pitch, joint_increment_value = 0.05, done_reward = -1000.0, alive_reward=10.0,desired_yaw=2.3, weight_r1=1.0, weight_r2=1.0,weight_r3=1.0,weight_r4=1.0,weight_r5=1.0,weight_r6=1.2, discrete_division=10):
        ...
        self._list_of_observations = [
            "distance_from_desired_point",
                 ...
                 "base_yaw",
                 "interval",
                 "joint_states_ankle1_r",
                 "joint_states_ankle2_r",
                 "joint_states_pelvis_r",
                 "joint_states_j_tibia_r",
                 "joint_states_thigh2_r",
                 "joint_states_thigh1_r",
                 "joint_states_ankle1_l",
                 "joint_states_ankle2_l",
                 "joint_states_pelvis_l",
                 "joint_states_j_tibia_l",
                 "joint_states_thigh2_l",
                 "joint_states_thigh1_l"]
        ...
        rospy.Subscriber("/darwin/imu", Imu, self.imu_callback)
        ...
        rospy.Subscriber('/darwin/joint_states', JointState, self.joints_state_callback)
    ...
    def check_all_systems_ready(self):
        ...
        while imu_data is None and not rospy.is_shutdown():
            try:
                imu_data = rospy.wait_for_message("/darwin/imu", Imu, timeout=0.1)
        ...
        while joint_states_msg is None and not rospy.is_shutdown():
            ...
            try:
                ...
                joint_states_msg = rospy.wait_for_message("/darwin/joint_states", JointState, timeout=0.1)
    ...
    def get_distance_from_point(self):
        ...
        distance = self.base_position.x

        return distance
    
    def get_time_maded(self, p_end):
        """
        Given a Vector3 Object, get distance from current position
        :param p_end:
        :return:
    ...
    def joints_state_callback(self,msg):
        ...
        #rospy.loginfo("Message joints:"+str(self.joints_state))


    def darwin_distance(self):
        
        #rospy.loginfo("COso x:"+str(self.base_position.z))
        #rospy.loginfo("COso y:"+str(self.desired_world_point.z))
        distance_ok = self.base_position.z >= self.desired_world_point.z
        return distance_ok
    
    def darwin_interval(self):

        interval_ok = self._time_step <= self._interval
        return interval_ok
    ...
    def calculate_reward_joint_position(self, weight=-5.0):
        """
        ...
        for joint_pos in self.joints_state.position:
            ...
            acumulated_joint_pos += weight *(joint_pos*joint_pos)+3
            rospy.loginfo("calculate_reward_joint_position>>joint_pos=" + str(joint_pos))
            rospy.loginfo("calculate_reward_joint_position>>reward_joint_pos=" + str(weight *(joint_pos*joint_pos)+3))
        reward = weight * acumulated_joint_pos
        rospy.loginfo("calculate_reward_joint_position>>reward=" + str(reward))
        return reward

    def calculate_reward_joint_effort(self, weight=-0.4):
        """
        ...
        for joint_effort in self.joints_state.effort:
            ...
            acumulated_joint_effort += weight *(joint_effort*joint_effort)+0.2
        ...
        reward =  acumulated_joint_effort
    ...
    def calculate_reward_distance_from_des_point(self, weight=5.0):
        """
        ...
        if(distance>1. and self.base_position.z>self.desired_world_point.z):
            reward = 3000
        elif(distance>0.0):
            reward = weight * distance
        else:
            reward=-10.0
        rospy.loginfo("calculate_reward_distance>>reward=" + str(reward))
        rospy.loginfo("Distance=" + str(distance))
        return reward

    def calculate_reward_interval(self, weight=1.0):
        if((self._interval - self._time_step)>0.0):
            reward= (self._interval - self._time_step)*weight
        else:
            #rospy.loginfo("calculate_reward_interval>>reward=" + str(reward))
            reward= -200.0
        
        rospy.loginfo("calculate_reward_interval>>reward=" + str(reward))
        rospy.loginfo("INterval=" + str(self._interval ))
        rospy.loginfo("time maded=" + str(self._time_step ))
    ...
    def calculate_total_reward(self):
        ...
        r6 = self.calculate_reward_interval(self._weight_r6)
    ...
    def get_observations(self):
        ...
        distance_from_desired_point = self.get_distance_from_point()
        ...
        for j in joint_states.position:
            if con==0 or con==1 or con==2 or con==3 or con==11 or con==12 or con==19 or con==20 or con==17 or con==18 or con==15 or con==16: 
                rospy.loginfo(str(joint_states.name[con])+">>"+str(joint_states.position[con]))
            
            con+=1
        
        joint_states_ankle1_r = joint_states.position[1]
        joint_states_ankle2_r = joint_states.position[3]
        joint_states_pelvis_r = joint_states.position[12]
        joint_states_j_tibia_r = joint_states.position[20]
        joint_states_thigh2_r = joint_states.position[18]
        joint_states_thigh1_r = joint_states.position[16]

        joint_states_ankle1_l = joint_states.position[0]
        joint_states_ankle2_l = joint_states.position[2]
        joint_states_pelvis_l = joint_states.position[11]
        joint_states_j_tibia_l = joint_states.position[19]
        joint_states_thigh2_l = joint_states.position[17]
        joint_states_thigh1_l = joint_states.position[15]
        ...
        for obs_name in self._list_of_observations:
            ...
            elif obs_name == "interval":
                observation.append(interval)
            elif obs_name == "joint_states_ankle1_r":
                observation.append(joint_states_ankle1_r)
            elif obs_name == "joint_states_ankle2_r":
                observation.append(joint_states_ankle2_r)
            elif obs_name == "joint_states_pelvis_r":
                observation.append(joint_states_pelvis_r)
            elif obs_name == "joint_states_j_tibia_r":
                observation.append(joint_states_j_tibia_r)
            elif obs_name == "joint_states_thigh2_r":
                observation.append(joint_states_thigh2_r)
            elif obs_name == "joint_states_thigh1_r":
                observation.append(joint_states_thigh1_r)

            elif obs_name == "joint_states_ankle1_l":
                observation.append(joint_states_ankle1_l)
            elif obs_name == "joint_states_ankle2_l":
                observation.append(joint_states_ankle2_l)
            elif obs_name == "joint_states_pelvis_l":
                observation.append(joint_states_pelvis_l)
            elif obs_name == "joint_states_j_tibia_l":
                observation.append(joint_states_j_tibia_l)
            elif obs_name == "joint_states_thigh2_l":
                observation.append(joint_states_thigh2_l)
            elif obs_name == "joint_states_thigh1_l":
                observation.append(joint_states_thigh1_l)
            else:
    ...
    def get_state_as_string(self, observation):
        ...
        rospy.loginfo("Inputs info:"+string_state)
    ...
    def get_action_to_position(self, action):
        ...
        if action == 0:  # Incrementar la primera posición
        ...
        elif action == 1:  # Decrementar la primera posición
            action_position[0] = joint_states_position[0] - self._joint_increment_value
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 2:  # Incrementar la segunda posición
            ...
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 3:  # Decrementar la segunda posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1] - self._joint_increment_value
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 4:  # Incrementar la tercera posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            ...
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 5:  # Decrementar la tercera posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2] - self._joint_increment_value
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 6:  # Incrementar la cuarta posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            ...
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 7:  # Decrementar la cuarta posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3] - self._joint_increment_value
            action_position[4] = joint_states_position[4]
            action_position[5] = joint_states_position[5]
        elif action == 8:  # Incrementar la quinta posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            ...
            action_position[5] = joint_states_position[5]
        elif action == 9:  # Decrementar la quinta posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
            action_position[4] = joint_states_position[4] - self._joint_increment_value
            action_position[5] = joint_states_position[5]
        elif action == 10:  # Incrementar la sexta posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
        ...
        elif action == 11:  # Decrementar la sexta posición
            action_position[0] = joint_states_position[0]
            action_position[1] = joint_states_position[1]
            action_position[2] = joint_states_position[2]
            action_position[3] = joint_states_position[3]
    ...
    def process_data(self):
        """
        Haz una funcion que le de un premio o castigo
        dependiendo de si lo que saca la IA
        es igual a la posicion asumida en la simulacion
        We return the total reward based on the state in which we are in and if its done or not
        ( it fell basically )
        :return: reward, done
        """
        darwin_height_ok = self.darwin_distance()
        darwin_interval_ok = self.darwin_interval()
        darwin_orientation_ok = self.monoped_orientation_ok()
        rospy.loginfo("Sigue?"+str(darwin_orientation_ok))
        done = not (darwin_orientation_ok  and darwin_height_ok)
        if done:
